...
namespace DVLD.Forms
{
    ...
    {
        public delegate void PersonDataUpdatedHandler(clsContact updatedPerson);
        public event PersonDataUpdatedHandler PersonDataUpdated;
        ...
        public AddEditPerson(int ID) : this()
        {
            Combobox1fill();
            Dt1MaxDateDate();
            this.PersonID = ID;
        ...
        private void _LoadData()
        {
            ...
            copiedImagePath = Person.ImagePath;
            if (!string.IsNullOrEmpty(Person.ImagePath) && File.Exists(Person.ImagePath))
            {
                PersonImage.ImageLocation = Person.ImagePath;
            }
            else
            {
                PersonImage.Image = null; // Ensure the PictureBox is cleared if no image exists
            }
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        using (FileStream fs = new FileStream(copiedImagePath, FileMode.Open, FileAccess.Read))
                        {
                            ...
                            PersonImage.ImageLocation = copiedImagePath;
                        }
        ...
        private void btnSave_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Person.ImagePath = PersonImage.ImageLocation;
                }
                else
                {
                    Person.ImagePath = "";
                ...
                if (Person.Save())
                {
                    PersonDataUpdated?.Invoke(Person);
